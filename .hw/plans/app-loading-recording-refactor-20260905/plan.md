---
generated_by: fable
draft_generated_by: codex
fable_reissued_on: 2026-09-05
...
### Fable再発行の記録（2026-09-05）

- 原稿はCodexが作成し、ユーザーが「codexで完成させて。」で例外承認した。Fableは本文・execution-context.md・verification-contract.mdの全文をレビューし、R00–R11の順序・依存・検証契約・hash/祖先条件に矛盾がないことを確認したうえで再発行した。
- 再発行はプラン本文のみを入力とし、コードは再読していない。行番号はすべてCodex調査時の基準HEADのものであり、実装時にシンボルと本文で同定する既定は変わらない。
- Fableが加えた修正は次の6点で、契約の意味は変えていない。(1) Fable可用性に関する記述を現状に合わせた。(2) R00手順3を「再採用済みの確認」に置換した。(3) R01のbase-commit更新の担当・時点・確認条件を明記した。(4) R01の横断検索世代refが無い場合の扱いを一文追加した。(5) R03の`build_request`/`send`引数振り分けの注意を追加した。(6) R11の担当分担を明記した。verification-contract.mdは無変更。
- 付録9の sml-decision.json / runtime-decision.json にある `generated_by: codex` は判定原稿の作成者を示す。FableはS/M/L=L、R軸=primeの判定を確認し採用した。
...
- 原稿作成時、Fableは組織の月間利用上限で起動できず、ユーザーは後続メッセージ「codexで完成させて。」で**原稿作成者をCodexとする例外**を明示承認した。2026-09-05に制限外環境でFable認証（`loggedIn: true`）を確認し、Fableが本文を再発行した。frontmatterの`generated_by: fable`は実際の再発行結果であり、Codex原稿は`codex-plan.md`として同ディレクトリに保管する。
- この承認と再発行は `.hw` の機械ゲート改変や実装後のFableレビュー省略の承認ではない。現行 `pr-ready-gate.sh` はFable生成frontmatterを要求し、本再発行がそれを満たす。R01開始時のbase-commit更新に伴うFable再確認と、R11のFableレビューは別途実施する。利用制限が再発した場合は該当時点で「実装は進行、Fable工程が未充足」と報告して中断する。ゲートの書換え・名前だけの付替えは禁止。
...
3. **機械ゲートとの整合**：Fable再採用は2026-09-05に完了済み（本文書がその結果）。引継ぎ担当は手順2のcopy前に、元rootのplanディレクトリで本再発行文を`plan.md`として保存し、Codex原稿全文を`codex-plan.md`へ移す。worktreeのplanディレクトリに`plan.md`（`generated_by: fable`）、`codex-plan.md`、`verification-contract.md`、`sml-decision.json`、`runtime-decision.json`、`execution-context.md`、`base-commit`（`67ea03210c2de4c8723780402d302948b138d939`＋末尾改行）が揃っていることを確認する。欠けていれば付録9と原稿から復元する。この段階でFableを再呼出しする必要はない。フック・reviewer名・gateを偽装しない。
...
**変更**：独立した`// Initial load`のeffectを削除し、既存のdropdown filter effectを唯一の初回取得元として残す。applyFiltersの引数・300ms検索debounce・取得条件・page数は変更しない。unmount専用cleanupでdebounceRefのtimerをclearし、transcriptSearchGenerationRefを増やして旧検索結果を無効化する（横断検索の世代refが既存になければ同名refを新設し、横断検索結果を適用する直前に世代一致を確認する）。検索中のdropdown動作の全面再設計はしない。
...
R00の基準commit後、prime起動前に、親担当がFable plannerへread-onlyで次を渡す。「`git diff --stat 67ea03210c2de4c8723780402d302948b138d939..$BASELINE_COMMIT` が `.hw/plans/app-loading-recording-refactor-20260905/` 配下と新設test 3ファイルだけであることを確認し、計画本文を再確認して、実装対象をR01–R10、最終証拠をR11として採用」。差分がその範囲を外れる場合、または本文の意味変更が必要と指摘された場合は実装せず報告する。再確認できた場合に限り、実行役はR01の作業で`base-commit`をR00のSHAへ更新し、`adoption.json`へ実際の開始HEAD・採用日時・原稿のgenerated_by=codex・ユーザー承認文・Fable再確認の結果を記録する。R01のcommitにはこの小さい計画起点更新も含める。これにより大きいHTML/全体索引は最終コードレビューの差分に入らない。
...
`request_kwargs`のうち`build_request`が受け付けないもの（例：`follow_redirects`）は`send`側へ渡し、`timeout`は`build_request`側へ渡す。両者が受け付ける引数名はR00で実際に入ったhttpx versionの署名で確認し、推測で振り分けない。
...
**担当**：手順1–2と6–7は実行役、手順3–5（Fableレビューの実行と判定の受領）は親担当。実行役は手順2完了時点で停止し、R10のSHAと検証証拠を親担当へ引き渡す。親担当がREADYを受領した後に実行役が手順6以降を行う。実行役がFableレビューを代行・自己宣言することは禁止。
...
原稿はCodexが作成しユーザーが明示承認済み、2026-09-05にFableが再発行済みです。frontmatterの generated_by: fable はその事実で、機械ゲートはその承認で変更されていません。R01開始時のbase-commit更新に伴うFable再確認と、R11のFableレビューを実施し、利用不可なら該当ゲートで停止して理由を報告してください。ゲートや署名を偽装しないでください。
...
会話履歴や本ディレクトリの他ファイルを受け取れなくても実行仕様は欠けない。R00を始める前に、このplan.mdをリポジトリの`.hw/plans/app-loading-recording-refactor-20260905/plan.md`へ保存する。同じディレクトリに、以下3ブロックをそれぞれ記載ファイル名でそのまま保存する。`base-commit`には調査SHA `67ea03210c2de4c8723780402d302948b138d939`と末尾改行を入れる。これは文書の復元であり、レビュー済みの記録ではない。R01開始時のbase-commit更新に伴うFable再確認は別途必須。HTML/索引/調査ノートは補助資料で、無い場合に実行を止める理由にはしない。下記JSONの`generated_by: codex`は判定原稿の作成者を示し、Fableは本再発行でその判定を確認・採用した。

...
| AT-006 | 切替後成果物(`cloudrun-after.json`, `health-after*.json`, `config-after.json`, `shared-login.txt`, `browser-network.txt`, `restart-test.txt`, `pm-assist-after.txt`, `cloudbuild.txt`, `docs/ops/cloudflare-named-tunnel.md`)で `grep -ci trycloudflare` が 0。AT-001 の切替前証明(`cloudrun-before.json`, `revisions-before.json`, `preflight.txt`)は対象外とし、非0ファイルと件数を実測記録する | command | `evidence/trycloudflare-grep.txt` |
...
| AT-010 | ブラウザで Cloud Run URL を開き、共有ログイン後に `/meetings` が描画される。Playwright `request` event または DevTools Network の実リクエスト捕捉で `trycloudflare` へのリクエスト 0 件 | browser | `evidence/browser-meetings.png`, `evidence/browser-network.txt` |

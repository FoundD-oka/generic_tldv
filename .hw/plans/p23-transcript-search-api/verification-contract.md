# Verification Contract — p23-transcript-search-api(Stage 1)

親契約 `.hw/plans/p23-transcript-fulltext-search/verification-contract.md` の
Stage 1 の表を、合格ライン不変のまま抽出したもの(項目 ID は親と同一)。
対象: `base-commit(7c3c6555f63950eaf5a496da07b6a84da62fd0e8)..HEAD` の差分。
検証は commit 済み clean tree に対して実行する。Stage 2(UI)の項目は本契約に
含まない(`p23-transcript-search-ui` の契約で判定)。
...
`.hw/gates/p23-transcript-search-api/` の証跡を確認。

## 監査 ID → 検証項目の対応

| 監査 ID | 要求 | 検証先 |
|---|---|---|
| FT-3 | 会議横断の全文検索(現状タイトル ILIKE のみ、meetings.py:1426-1438 系)のサーバ側実体 | AT-301〜308 |
| 依頼者指定 | 既存認可境界の維持(他ユーザーの文字起こしが検索で漏れない) | FP-301 |

## ベースライン取得手順(転記値の使用禁止)

着手時 base-commit の clean tree で python3.11 fresh venv を
...
数値と failed テスト名を本契約の改訂履歴へ記入して commit する。
api-gateway も同様に既存テストのベースラインを取る。
→ **実測済み。本契約末尾の改訂履歴に記録あり。**
...
| FP-302 | 変更ファイルが plan.md 記載の Stage 1 集合のみ。判定コマンドは `git diff --name-only <base>..HEAD -- ':(exclude).hw/plans'`(`.hw/plans/` は許容。それ以外の `.hw/`(hooks / rules / verify.sh 等)は除外しない) | diff | Fable | diff |
...
| FP-304 | meeting-api・api-gateway 既存テスト非退行: ベースライン failed/errors 集合に対し新規 fail 0(passed 増は新設テスト分のみ) | 同一 venv・同一コマンドでサマリ比較 | ゲート + Fable(改訂履歴の実測値と照合) | 両サマリ全文 |
...
- evidence pack required: yes(`.hw/gates/p23-transcript-search-api/` へ)
- hash-bound approval required: yes(M のため Fable 契約レビュー必須)
- research brief required: no(親プランのリサーチ節に実測記録済み)
...
## Research Freshness Checks(実測結果は本契約の改訂履歴に記入済み)
...
| RF-303 | CREATE INDEX CONCURRENTLY がトランザクション外実行を要すること(psycopg2 autocommit)。20260708 スクリプトの実装を踏襲して確認 | migration スクリプトの status コマンド実測 + 既存スクリプトとの diff 比較 | `.hw/gates/` に実行ログ |
...
親契約 `p23-transcript-fulltext-search` から Stage 1 の項目(AT-301〜308 /
FP-301〜306 / NFT-301〜302 / RF-301〜304)を項目 ID・合格ライン不変のまま抽出。
FP-304 は親の「meeting-api 既存テスト非退行」に api-gateway を明記(実測済み
ベースラインが両スイートにあるため。緩和ではなく明確化)。以下の実測記録は
実装者が親契約へ記入したものを欠落なく引き継いだ。
...
### Stage 1 実装時実測(2026-08-09、implementer 実施)

 ...
 *     通す抜け道を塞ぐため。
 *   - lint-baseline.json を変更する PR は、commit message または PR 本文に
 *     由来を記録すること: (1) 測定コマンド、(2) node と eslint のバージョン、
 *     (3) 実測した errors / warnings の合計値。
 *     過小方向(実測より小さく書く)はこのラチェットの「増加で赤」で CI が
 *     機械検出するが、過大方向(水増しして新規違反を隠す)は CI では捕まらない。
 *     過大方向はその PR のレビューでこの記録と突合して判定する。
 *
 * 依存は Node 標準モジュールのみ。
 */

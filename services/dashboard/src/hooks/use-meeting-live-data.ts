...
export function useMeetingLiveData(props: Props) {
 ...
 const pollArtifacts = postMeeting && currentMeeting?.data?.recording_enabled !== false && !props.hasRecordingAudio && !props.audioResolutionError;
 ...
 useEffect(() => { if (pollArtifacts) return; if (isBrowserSession && status !== "stopping" && status !== "completed") return; if (platform && nativeId) { void props.fetchTranscripts(platform, nativeId, numericId); void props.fetchChatMessages(platform, nativeId); } }, [status, shouldUseWebSocket, isBrowserSession, pollArtifacts, platform, nativeId, numericId, props.fetchTranscripts, props.fetchChatMessages]);

...
describe("loading and playback characterization", () => {
  ...
  it("R00 playback keeps fragment seek coordinates", async () => {
    ...
    await act(async () => playback.handleSegmentClick(3, segmentB.absolute_start_time));
    expect(audio.seekToFragment).toHaveBeenCalledWith(1, 3);
    expect(video.seekTo).toHaveBeenCalledWith(15);
    ...
    (playback.videoPlayerRef as { current: VideoPlayerHandle | null }).current = video;
    await act(async () => playback.handleSegmentClick(3, segmentB.absolute_start_time));
    expect(singleAudio.seekTo).toHaveBeenCalledWith(3);
    expect(video.seekTo).toHaveBeenLastCalledWith(3);
  });

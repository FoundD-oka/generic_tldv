...
describe("meetings proxy contract", () => {
  ...
  it.each(["headers", "body"])("R02 headers and body hangs both time out (%s)", async (phase) => {
    ...
    await expect(response.json()).resolves.toEqual({ error: "Request timeout", retryable: true });
  ...
  it.each([
    ...
    await expect(response.json()).resolves.toEqual({ error: "Invalid meetings response", retryable: true });
    expect(response.headers.get("cache-control")).toBe("no-store");
  });

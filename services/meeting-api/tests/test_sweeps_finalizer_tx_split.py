...
async def test_finalizer_builds_masters_outside_tx_and_writes_by_media_file_id():
    ...
    def _fake_mux(storage, video, audio):
        # 動画は混合音声の master と合成される(video-audio-mux)。これも
        # read tx が閉じた後の I/O 相で、かつ音声 master 確定後に走ること。
        assert order and order[0] == "rollback"
        assert audio["storage_path"] == f"{base}/audio/master.wav"
        assert audio["finalized_by"] == "recording_finalizer.master"
        order.append(f"io:{video['id']}")
        return {
            **video,
            "source_video_path": video["storage_path"],
            "storage_path": f"{base}/video/master.av.webm",
            "video_audio_mux": {"version": 1},
            "finalized_by": "recording_finalizer.master",
            "is_final": True,
        }
    ...
    with patch.object(fin, "create_storage_client", return_value=MagicMock()), \
         ...
         patch.object(fin, "mux_recording_video", new=_fake_mux), \
    ...
    assert media_files[2]["storage_path"] == f"{base}/video/master.av.webm"
    assert media_files[2]["source_video_path"] == f"{base}/video/000000.webm"
    assert media_files[2]["is_final"] is True
    assert media_files[2]["finalized_by"] == "recording_finalizer.master"
...
async def test_finalizer_keeps_no_fallback_and_idempotent_contract():
    ...
    音声と同じ録音にある動画は混合音声との合成対象になる(video-audio-mux)ため、
    冪等性の検証用の動画は動画のみの録音に置く。
    """
    order: list = []
    ...
    media_files = {
        mf["id"]: mf
        for rec in meeting.data["recordings"]
        for mf in rec["media_files"]
    }
